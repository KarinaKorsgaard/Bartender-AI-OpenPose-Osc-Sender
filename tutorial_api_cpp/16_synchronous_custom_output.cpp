...
public:
	...
	void setupOsc() {
		op::log("Setup osc...", op::Priority::High);
	...
	void sendOsc(const std::shared_ptr<std::vector<std::shared_ptr<op::Datum>>>& datumsPtr, UdpTransmitSocket& socket, osc::OutboundPacketStream& p) {
		const auto& poseKeypoints = datumsPtr->at(0)->poseKeypoints;
		for (auto person = 0; person < poseKeypoints.getSize(0); person++)
		{
			std::string adress = "/person" + std::to_string(person);
			const char *a = adress.c_str();
    ...
    void workConsumer(const std::shared_ptr<std::vector<std::shared_ptr<op::Datum>>>& datumsPtr)
    {
        try
        {
            ...
            {
				sendOsc(datumsPtr, transmitSocket, p);
                ...
                op::log("BLABLABLALBLABLALB keypoints: " + datumsPtr->at(0)->faceKeypoints.toString());
...
void configureWrapper(op::Wrapper& opWrapper)
{
    try
    {
        ...
        // netInputSize
        const auto netInputSize = op::flagsToPoint(FLAGS_net_resolution, "-1x80");
        // faceNetInputSize
